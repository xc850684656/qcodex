...
static void stream_update_ts(SyncQueue *sq, unsigned int stream_idx, int64_t ts)
{
    ...
    if (st->limiting &&
        (sq->head_stream < 0 || sq->head_stream == (int)stream_idx))
...
static int overflow_heartbeat(SyncQueue *sq, int stream_idx)
{
    ...
    if (stream_idx < 0) {
        ...
        for (int i = 0; i < (int)sq->nb_streams; i++) {
            st = &sq->streams[i];
            if (st->head_ts != AV_NOPTS_VALUE &&
                (ts == AV_NOPTS_VALUE ||
...
static void offset_audio(AVFrame *f, int nb_samples)
{
    ...
    for (int i = 0; i < planes; i++) {
        ...
        if (i < (int)FF_ARRAY_ELEMS(f->data))
...
static int frame_is_aligned(const SyncQueue *sq, const AVFrame *frame)
{
    ...
    if (!((uintptr_t)frame->data[0] & sq->align_mask) &&
        ...
        frame->linesize[0] > (int)sq->align_mask)
        return 1;
                           ...
                           AVFrame *dst, int nb_samples)
{
    ...
    av_assert0(st->samples_queued >= (uint64_t)nb_samples);
                              ...
                              SyncQueueFrame frame)
{
    ...
    if (av_fifo_can_read(st->fifo) &&
        (st->frame_samples <= (int)st->samples_queued || st->finished)) {
        ...
        if (st->finished)
            nb_samples = FFMIN(nb_samples, (int)st->samples_queued);
        ...
        if (cmp <= 0 || ts == AV_NOPTS_VALUE || !sq->have_limiting) {
            if (nb_samples &&
                ...
                av_fifo_drain2(st->fifo, 1);
                av_assert0((int)st->samples_queued >= frame_samples(sq, frame));
...
static int receive_internal(SyncQueue *sq, int stream_idx, SyncQueueFrame frame)
{
    ...
    return (nb_eof == (int)sq->nb_streams) ? AVERROR_EOF : AVERROR(EAGAIN);
}
